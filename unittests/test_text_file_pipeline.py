...
engine2 = global_init(conn_str, config.database, 'NORMAL')
pipeline2 = TextFilePipeline(engine2, "DEBUG")

def test_text_to_dataframe():
    result = pipeline2.text_to_dataframe(bucket_name, key)
...
engine3 = global_init(conn_str, config.database, 'NORMAL')
pipeline3 = TextFilePipeline(engine3, "DEBUG")

def test_transform_string_to_int():
    df = pipeline3.text_to_dataframe(bucket_name, key)
    result = pipeline3.transform_string_to_int(df, ['psychometrics', 'psychometrics_max',
